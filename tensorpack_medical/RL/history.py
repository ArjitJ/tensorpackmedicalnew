...
class HistoryFramePlayer(ProxyPlayer):
    ...
    def __init__(self, player, hist_len, concat_axis=2):
        """
        Args:
            hist_len (int): total length of the state, including the current
                and `hist_len-1` history.
            concat_axis (int): concatenation axis - 2 or 3 for 2d or 3d states
                (default is 2)
        """
        super(HistoryFramePlayer, self).__init__(player)
        self.history = HistoryBuffer(hist_len, concat_axis)
