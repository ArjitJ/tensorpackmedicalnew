...
def play_n_episodes(player, predfunc, nr, render=False):
    ...
    for k in range(nr):
        ...
        logger.info(
            "{}/{} - {} - score {} - distError {} - q_values {}".format(
                k + 1, nr, filename, score, ditance_error, q_values
            )
        )
...
def eval_with_funcs(
    predictors, nr_eval, get_player_fn, directory=None, files_list=None
):
    """
    Args:
        ...
        def run(self):
            with self.default_sess():
                player = get_player_fn(
                    directory=directory, train=False, files_list=files_list
                )
